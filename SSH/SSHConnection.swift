...
private final class SshReceiveBuffer: Sendable {
    nonisolated private static let maxBufferedBytes = 1_048_576
    nonisolated private static let compactionThreshold = 8_192

    private struct State: Sendable {
        var buffer = Data()
        var readIndex = 0
        var dataContinuation: CheckedContinuation<Void, Error>?
        var isClosed = false
    }
    ...
    nonisolated func shouldPauseReceiving() -> Bool {
    ...
    nonisolated func read(into ptr: UnsafeMutableRawPointer, maxLength: Int) -> Int {
        state.withLock { state in
    ...
    nonisolated func waitForData() async throws {
        try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Void, Error>) in
                let immediateResult: Result<Void, Error>? = state.withLock { state in
                    if state.buffer.count > state.readIndex {
                        return .success(())
                    }
                    if state.isClosed {
                        return .failure(SSHConnectionError.disconnectedOrTeardown)
                    }
                    state.dataContinuation = cont
                    return nil
                }
                if let immediateResult {
                    cont.resume(with: immediateResult)
                }
            }
        } onCancel: {
    ...
    nonisolated private func unreadByteCount(state: State) -> Int {
        state.buffer.count - state.readIndex
    }

    nonisolated private func compactBufferIfNeeded(state: inout State) {
...
private func withSSHTimeout<T: Sendable>(
    _ operation: String,
    seconds: Double,
    ...
    try await withThrowingTaskGroup(of: T.self) { group in
        ...
        group.addTask {
            ...
            throw SSHConnectionError.timedOut(operation)
        }
...
private actor SshSessionActor {
    ...
    func teardown() {
        ...
        contextRef = nil
        for task in waiting {
            task.continuation.resume(throwing: SSHConnectionError.disconnectedOrTeardown)
        }
    }
    ...
    func pingTasks() {
        ...
        let waiting = pendingTasks
        pendingTasks.removeAll()
        for task in waiting { task.continuation.resume(returning: ()) }
    }
    ...
    private func waitForRetry() async throws {
        ...
        try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Void, Error>) in
                pendingTasks.append(PendingTask(id: token, continuation: cont))
            }
        } onCancel: {
            Task {
                await cancelPendingTask(id: token)
            }
        }
    }
    ...
    private func cancelPendingTask(id: UUID) {
...
final class SSHConnection {
    ...
    func connect(
        ...
        do {
            ...
            try await withSSHTimeout("TCP connection", seconds: 15) {
                try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Void, Error>) in
        ...
        do {
            try await withSSHTimeout("SSH handshake", seconds: 20) {
                try await self.sshActor.handshake()
            }
            ...
            case .password(let pw):
                try await withSSHTimeout("SSH password authentication", seconds: 20) {
                    try await self.sshActor.authPassword(username: username, password: pw)
                }
            case .privateKey(let pem, let passphrase):
                try await withSSHTimeout("SSH key authentication", seconds: 20) {
                    try await self.sshActor.authPublicKey(username: username, privateKeyPEM: pem, passphrase: passphrase)
                }
            }

            // Open interactive shell
            try await withSSHTimeout("SSH shell setup", seconds: 15) {
                try await self.sshActor.openChannel()
                try await self.sshActor.requestPTY(cols: 80, rows: 24)
                try await self.sshActor.requestShell()
            }
    ...
    func send(_ data: Data) {
        Task { [weak self] in
            guard let self else { return }
            do {
                try await sshActor.sendToChannel(data)
            } catch {
                await failCurrentConnection(error)
            }
        }
    }
    ...
    func resize(cols: Int, rows: Int) {
        Task { [weak self] in
            guard let self else { return }
            do {
                try await sshActor.resizePTY(cols: cols, rows: rows)
            } catch {
                await failCurrentConnection(error)
            }
        }
    ...
    private func failCurrentConnection(_ error: Error) async {
        guard let connection = nwConnection else {
            transition(to: .failed(error.localizedDescription))
            return
        }
        await handleTransportClosed(for: connection, error: error.localizedDescription)
    }
